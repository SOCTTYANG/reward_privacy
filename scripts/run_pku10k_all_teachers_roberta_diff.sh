...
# Compare each PKU-10K two-stage RoBERTa substitute against its own target RM.
# Results are written to output/diff_pku10k_<teacher>_to_roberta/diff_summary.json.
...
MODEL_DIR="${MODEL_DIR:-/mnt/model_data/models}"
cd "${PROJECT_DIR}"
...
run_diff() {
  ...
  local student="${PROJECT_DIR}/output/pku10k_${teacher_name}_to_roberta"
  local output_dir="${PROJECT_DIR}/output/diff_pku10k_${teacher_name}_to_roberta"
  ...
  echo "======================================================"
  echo "Diff evaluation: ${teacher_name} target RM -> PKU10K RoBERTa"
  echo "======================================================"
...
echo "[OK] All four PKU-10K diff evaluations finished."
echo "[INFO] Summaries: output/diff_pku10k_*_to_roberta/diff_summary.json"
