...
REWARD_GAP_MIA_SCRIPT="${SCRIPT_DIR}/3_4_reward_gap_ablation_mia.py"
...
# Fixed reward model. Do not change this across pretrained LLM runs.
...
MODEL_TAGS=("llama2-7b" "llama2-13b-hf" "llama32-3b" "qwen3-8b" "mistral-7b-v0.1")
MODEL_PATHS=("${LLAMA2_7B}" "${LLAMA2_13B}" "${LLAMA32_3B}" "${QWEN3_8B}" "${MISTRAL_7B}")
...
OVERWRITE_STAGE34="${OVERWRITE_STAGE34:-${OVERWRITE}}"
...
require_file "${REWARD_GAP_MIA_SCRIPT}" "reward-gap ablation MIA script"
...
SUMMARY_CSV="${OUTPUT_ROOT}/mia_reward_gap_ablation_m${M_VALUE}_table_metrics.csv"
MANIFEST_TSV="${OUTPUT_ROOT}/mia_reward_gap_ablation_m${M_VALUE}_outputs.tsv"
printf "model_tag,ASR,AUC,T@1%%F,T@5%%F\n" > "${SUMMARY_CSV}"
printf "model_tag\ttable_csv\tsummary_json\trun_dir\n" > "${MANIFEST_TSV}"

echo "============================================================"
echo "[MIA Reward-Gap Ablation]"
echo "[M         ] ${M_VALUE}"
echo "[GPU       ] CUDA_VISIBLE_DEVICES=${CUDA_DEVICE}"
echo "[FIXED RM  ] ${RM_BASE_MODEL} + ${RM_ADAPTER}"
echo "[ABLATION  ] skip step 3; reward-gap-only score_mode=${SCORE_MODE}"
echo "[VARY      ] only --pretrained_llm_path in step 2"
...
run_one_model() {
  local model_tag="$1"
  local model_path="$2"
  local run_name="${model_tag}-m${M_VALUE}-reward-gap-ablation"
  local run_dir="${OUTPUT_ROOT}/${run_name}"
  local stage31_dir="${run_dir}/3.1-Target-Data"
  local stage32_dir="${run_dir}/3.2-Candidate-Response-Generation"
  local stage34_dir="${run_dir}/3.4-Reward-Gap-Ablation-MIA"
  ...
  local table_csv="${stage34_dir}/3.4_reward_gap_ablation_table_metrics.csv"
  local summary_json="${stage34_dir}/3.4_reward_gap_ablation_summary.json"

  mkdir -p "${stage31_dir}" "${stage32_dir}" "${stage34_dir}"
  ...
  echo "============================================================"
  echo "[3.4] Reward-gap-only membership inference"
  echo "============================================================"
  if [[ -s "${table_csv}" && "${OVERWRITE_STAGE34}" != "1" ]]; then
    echo "[3.4] Found existing table metrics, skip: ${table_csv}"
  else
    CUDA_VISIBLE_DEVICES="${CUDA_DEVICE}" "${PYTHON_BIN}" "${REWARD_GAP_MIA_SCRIPT}" \
  ...
  printf "%s\t%s\t%s\t%s\n" "${model_tag}" "${table_csv}" "${summary_json}" "${run_dir}" >> "${MANIFEST_TSV}"
