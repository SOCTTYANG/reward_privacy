...
# Run the four missing Defender Evaluation-10K Stage-A + teacher-distillation Stage-B
...
export HF_HOME="${HF_HOME:-/path/to/code-data/cache/huggingface}"
export HF_DATASETS_CACHE="${HF_DATASETS_CACHE:-${HF_HOME}/datasets}"
export TRANSFORMERS_CACHE="${TRANSFORMERS_CACHE:-${HF_HOME}/transformers}"
export TORCH_HOME="${TORCH_HOME:-/path/to/code-data/cache/torch}"
...
if [[ -z "${STUDENT_MODEL:-}" ]]; then
  ...
  elif [[ -d "/path/to/models/roberta-base" ]]; then
    STUDENT_MODEL="/path/to/models/roberta-base"
  elif [[ -d "/path/to/code-data/models/roberta-base" ]]; then
    STUDENT_MODEL="/path/to/code-data/models/roberta-base"
  else
    STUDENT_MODEL="${PROJECT_DIR}/models/roberta-base"
  fi
fi
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/logs/defender_evaluation_teacher_scaling_roberta}"
...
run_experiment() {
  ...
  local output_dir="${OUTPUT_ROOT}/defender_evaluation_${teacher_name}_to_roberta"
  local metrics_path="${output_dir}/two_stage_extracted_rm_metrics.json"
  local log_path="${LOG_DIR}/defender_evaluation_${teacher_name}_to_roberta.log"

  for required_path in \
    "${STUDENT_MODEL}" \
    "${PROJECT_DIR}/data/defender_evaluation_pref_train.jsonl" \
    "${PROJECT_DIR}/data/defender_evaluation_pref_eval.jsonl" \
  ...
  echo "======================================================"
  echo "Defender Evaluation-10K two-stage extraction: ${teacher_name} -> RoBERTa-base"
  echo "Stage A: Defender Evaluation-10K preference data | Stage B: teacher distillation"
...
echo "[OK] All four Defender Evaluation-10K RoBERTa-base extraction runs finished."
