"""Flatten a disjoint slice of Attacker Preference preference triples into teacher queries."""
    ...
    with open(path, encoding="utf-8") as handle:
        for line in handle:
            ...
            if not {"prompt", "chosen", "rejected"} <= row.keys():
                raise KeyError("Attacker Preference records must contain prompt, chosen, and rejected.")
...
def main(args) -> None:
    ...
    if len(selected) != args.max_pairs:
        raise ValueError(
            ...
            f"but only {len(selected)} are available from {len(pairs)} valid Attacker Preference pairs."
        )
    ...
    print(f"[OK] selected {len(selected)} disjoint Attacker Preference triples")
...
def parse_args():
    ...
    parser.add_argument("--skip_pairs", type=int, required=True,
                        help="Number of valid Attacker Preference triples reserved for Stage A.")
    parser.add_argument("--max_pairs", type=int, required=True,
                        help="Number of subsequent Attacker Preference triples to flatten for Stage B.")
