...
def main():
    ...
    data=load_records(a.input_path); labels=[]; scores=[]
    for row in data:
        ...
        gap=float(row["r_plus"])-float(row["r_minus"]); norm=float(row["grad_norm"])
    ...
    save_json(data,a.output_path); save_json(report,a.summary_path)
